...
PROGRAM data2timedata
 ...
 do while ( (time .eq. datain%TDtime(1) ) .or. &
    ...
    write(u_number,*,iostat=res) datain%ndata, '  ! nnz'
     ...
     do j=1,datain%ndata
        write(u_number,*,iostat=res) j,(factor*datain%TDactual(k,j),k=1,datain%dimdata)
        if(res .ne. 0) THEN
           write(rdwr,'(i5)') j
           str=etb(rdwr)//'/'
...
subroutine err_handle(lun_err, narg,command_line)
  ...
  write(lun_err,*) '  scale_timedata.out <filein> <factor> <product>'
  write(lun_err,*) ' where : '
  write(lun_err,*) ' filein  (in ) : timedata in ASCII'
  write(lun_err,*) ' factor  (in ) : real'
