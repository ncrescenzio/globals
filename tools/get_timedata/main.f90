...
PROGRAM data2timedata
  ...
  ! read inputs
  call datain%set(stderr, filein,time2read,end_reached)
  steady = datain%steadyTD
    
  write(u_number,*,iostat=res) 'time', time2read
  write(u_number,*,iostat=res) datain%ndata, '  ! nnz'

  do j=1,datain%ndata
     write(u_number,*,iostat=res) j,(datain%TDactual(k,j),k=1,datain%dimdata)
     if(res .ne. 0) THEN
        write(rdwr,'(i5)') j
        str=etb(rdwr)//'/'
